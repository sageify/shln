...
g() {
  ...
  --menu | -m)
    g -l | cat -n
    read -p "Edit repo number(s): " lines
    for line in $lines; do
      [ "$line" -eq "$line" ] 2>/dev/null && __=$(g -l | sed $line'!d') && g -e "$__"
    done
    ;;
  -l)
  ...
  --find | -f) grm_cd && find . -path "./${2-*}" | cut -c 3- ;;
...
EOF
    ;;
  -* | --*) echo "grm: $1: Option not found" 1>&2 && return 1 ;;
  *)
    case "$1" in
    --) shift && g -e "$@" ;;
    *) g -e "$@" ;;
